...
public class StorageTest {
	static void test(Storage storage) {
		int testNum = 1000000;
		...
		for (int i = 0; i < testNum; i++) {
			...
			try {
				if (randRate < 80) {
					...
					long key = genRandKeyWithSeed(random, keySeeds);
					c3++;
					storage.put(key, enbyteLong(key * 2));
				}
	...
	static void test2(Storage storage) {
		...
		for (int i = 0; i < testNum; i++) {
			...
			try {
				if (randRate < 70) {
					storage.get(genRandKeyWithoutSeed(random, keySeeds));
				} else if (randRate < 72) {
					storage.free(genRandKeyWithoutSeed(random, keySeeds));
				} else if (randRate < 85) {
					long key = genRandKeyWithoutSeed(random, keySeeds);
					storage.put(key, enbyteLong(key * 2));
				} else if (randRate < 90) {
					long key = genRandKeyWithSeed(random, keySeeds);
					ByteBuffer bb = storage.getByteBuffer(key);
					c1++;
					if (bb != null) {
						c2++;
					...
					storage.free(genRandKeyWithSeed(random, keySeeds));
				} else {
					long key = genRandKeyWithSeed(random, keySeeds);
					storage.put(key, enbyteLong(key * 2));
				}
	...
	static void test3(Storage storage) throws IOException, StorageException {
		int testNum = 10000000;

		long[] values = new long[700];
		for (int i = 0; i < 700; i++) {
			values[i] = 100000001;
		}
		...
		System.out.println("write start at:" + writeStartTime);
		for (int i = 0; i < testNum; i++) {
			long key = 10000000L + i;
			storage.put(key, bytes);
		}
		long writeEndTime = System.currentTimeMillis();
		System.out.println("write stop at:" + writeEndTime);
		System.out.println("write test|testNum: " + testNum + ",use "
				+ (writeEndTime - writeStartTime) + "ms, "
				+ (testNum / ((writeEndTime - writeStartTime) / 1000)) + "/s");

		long readStartTime = System.currentTimeMillis();
		System.out.println("read start at:" + readStartTime);
		for (int i = 0; i < testNum; i++) {
			long key = 10000000 + i;
			storage.get(key);
		}
		long readEndTime = System.currentTimeMillis();
		System.out.println("read stop at:" + readEndTime);
		System.out.println("test " + testNum + ",use "
				+ (readEndTime - readStartTime) + "ms, "
				+ (testNum / ((readEndTime - readStartTime) / 1000)) + "/s");
		System.out.println(storage);
	}
	...
	public static void main(String[] args) throws StorageException, IOException {
		Storage storage = new Storage(
				"/Users/zhouqian/mywork/java/cherry-storage/test", 10000001,
				6144);
